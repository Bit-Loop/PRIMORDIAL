    ...
    <div style={{ padding: '10px 12px', border: '1px solid var(--line)', borderLeft: `3px solid var(--${tone === 'violet' ? 'violet' : tone === 'green' ? 'green' : tone === 'red' ? 'red' : 'line-strong'})`, borderRadius: 'var(--r-2)', background: 'var(--bg-deep)', display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: '0 0 auto' }}>
          <button
            className={`btn sm ${p.gated ? '' : p.status === 'rejected' ? 'ghost' : 'primary'}`}
            disabled={p.status === 'rejected'}
            style={p.status === 'rejected' ? { opacity: 0.4 } : {}}
            onClick={() => API.command?.(p.gated ? 'review-interest' : 'view-interest', { interest_id: p.id, title: p.title })}
          >
            {p.gated ? '◆ REVIEW' : p.downloadable ? '↓ DOWNLOAD' : p.status === 'rejected' ? 'REJECTED' : 'VIEW'}
          </button>
          {p.evidence.length > 0 && <button className="btn ghost sm" onClick={() => API.command?.('open-interest-evidence', { interest_id: p.id, evidence_ids: p.evidence, title: `Open evidence for ${p.title}` })}>EVIDENCE</button>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="subtabs">
        ...
        <div style={{ flex: 1 }} />
        <span className="dim mono" style={{ fontSize: 10.5, paddingRight: 8 }}>{targetLabel} · active</span>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {sub === 'surfaces' && (
          <>
            ...
            <Panel title="ATTACK SURFACES" sub={`${filteredSurfaces.length} surfaces · ${targetLabel}`} className="fill">
              <table className="t">
        ...
        {sub === 'artifacts' && (
          <Panel title="AI ARTIFACTS &amp; TOOL OUTPUTS" sub={`${D.artifacts.length} items · ${targetLabel}`} className="fill" bodyClass="tight">
