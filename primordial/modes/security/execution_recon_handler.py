...
class PrimitiveReconHandlerMixin:
    def _handle_recon_scan(self, task: Task, context: ContextSlice) -> TaskExecutionResult:
        ...
        for plan in self._build_probe_plans(self._target_scope_assets(target)):
            self._process_recon_probe(
                task,
                target,
                result,
                plan,
        ...
        if not successful_probes:
            return self._recon_failure_result(task, target, result, errors)
        self._append_recon_success_outputs(
            task,
            target,
            result,
            successful_probes=successful_probes,
    ...
    def _process_recon_probe(
        ...
        *,
        ...
        self._append_recon_probe_evidence(task, target, result, plan, probe, content_paths, auth_candidates)

    def _append_recon_probe_evidence(
        self,
        task: Task,
        target,
        result: TaskExecutionResult,
        ...
        result.evidence.append(
            EvidenceRecord(
                ...
                metadata=self._recon_evidence_metadata(probe, content_paths, auth_candidates),
            )
        )

    def _recon_artifact_payload(self, plan: dict[str, object], probe: dict[str, object]) -> dict[str, object]:
        return {
    ...
    def _recon_failure_result(
        ...
        failure_artifact = self._write_artifact(task, target.id, "recon-failures", {"target": target.handle, "errors": errors})
    ...
    def _append_recon_success_outputs(
        self,
        task: Task,
        target,
        result: TaskExecutionResult,
        *,
        ...
        result.notes.append(self._recon_summary_note(task, target, successful_probes, auth_surfaces, discovered_paths, discovered_parameters))
        if auth_surfaces:
            result.interests.append(self._recon_auth_interest(task, target, result, auth_surfaces))
        interesting_paths = sorted(path for path in discovered_paths if any(keyword in path.lower() for keyword in AUTH_KEYWORDS))
        if successful_probes:
            result.findings.append(self._recon_reachable_finding(task, target, result, successful_probes, auth_surfaces, interesting_paths))
        result.handoffs.append(self._recon_handoff(task, result))
        result.events.append(
            EventRecord(
    ...
    def _recon_summary_note(self, task: Task, target, successful_probes: list[dict[str, object]], auth_surfaces: set[str], discovered_paths: set[str], discovered_parameters: set[str]) -> Note:
        return Note(
            target_id=target.id,
            task_id=task.id,
            title="Recon summary",
    ...
    def _recon_auth_interest(self, task: Task, target, result: TaskExecutionResult, auth_surfaces: set[str]) -> Interest:
        return Interest(
            target_id=target.id,
    ...
    def _recon_handoff(self, task: Task, result: TaskExecutionResult) -> TaskHandoff:
        return TaskHandoff(
            task_id=task.id,
