...
try:
    ...
    _TextualAppBase = object
else:
    _TEXTUAL_AVAILABLE = True
    _TextualAppBase = App[None]
...
class PrimordialTextualApp(_TextualAppBase):
    CSS = """
    Screen {
        layout: vertical;
    }
    ...
    def action_tick(self) -> None:
        self._runtime.run_tick()
        self._refresh_views()

    def action_compact(self) -> None:
        self._runtime.compact_memory()
        self._refresh_views()

    def action_process(self) -> None:
        self._runtime.process_external_queues()
        self._refresh_views()

    def action_approve_first(self) -> None:
        for task in self._runtime.dashboard().tasks:
            if task.status == TaskStatus.NEEDS_APPROVAL:
                self._runtime.approve_task(task.id, approved=True)
                break
        self._refresh_views()

    def _refresh_views(self) -> None:
        snapshot = self._runtime.dashboard()
        tasks = self.query_one("#tasks", DataTable)
        tasks.clear()
        ...
        summary.update(render_dashboard_text(self._runtime))
...
def launch_tui(runtime: PrimordialRuntime) -> int:
    ...
    PrimordialTextualApp(runtime).run()
    return 0
