...
class CTFLabPhaseCatalogTests(unittest.TestCase):
    ...
    def test_phase_seven_cloudgoat_tracks_control_contract_progress(self) -> None:
        phase = load_ctf_lab_phase_catalog(CATALOG_PATH).phase(7)
        commands = "\n".join(phase.validation_commands)

        self.assertEqual(phase.status, "in_progress")
        self.assertIn("sandbox_cloud_account_verified", phase.exit_gates)
        self.assertIn("account_boundary_and_region_scope_enforced", phase.exit_gates)
        self.assertIn("teardown_evidence_recorded", phase.exit_gates)
        self.assertEqual(phase.verified_environment_refs, ())
        self.assertEqual(phase.evidence_refs, ())
