    ...
    <div style={{
      ...
      {open && (
        <div style={{ marginTop: 8, display: 'flex', gap: 6 }}>
          ...
          <button className="btn ghost sm">ASK AI →</button>
          <button className="btn ghost sm">PIN</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="subtabs">
        ...
        <div style={{ flex: 1 }} />
        <span className="dim mono" style={{ fontSize: 10.5, paddingRight: 8 }}>pirate.htb · {D.methodology.label}</span>
      </div>
          ...
          <>
            ...
            <div style={{ flex: 1, minWidth: 0, overflow: 'auto', padding: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button className="btn ghost sm" style={{ marginLeft: 'auto' }}>+ PIN ITEM</button>
              </div>
              {pinned.length === 0 ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                ...
                <button className="btn ghost sm" style={{ marginLeft: 'auto' }}>+ ADD QUESTION</button>
              </div>
              ...
              {D.methodology.phases.map((ph, i) => {
                ...
                return (
                  <div key={ph.id} style={{ padding: '14px 14px 12px', border: `1px solid var(--${tone === 'gray' ? 'line' : tone})`, borderRadius: 'var(--r-2)', background: 'var(--bg-deep)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    {ph.status !== 'locked' && (
                      <button className="btn ghost sm" style={{ marginTop: 4 }}>VIEW TASKS</button>
                    )}
                  </div>
                );
              })}
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: 10 }}>
            ...
            <div style={{ marginTop: 10 }}>
              <div style={{ padding: '8px 10px', background: 'var(--green-soft)', border: '1px solid var(--green)', borderRadius: 'var(--r-2)', fontFamily: 'var(--mono)', fontSize: 11, display: 'flex', alignItems: 'center', gap: 8 }}>
                <Dot tone="green" />
                POLICY: DDoS forbidden · PoC exec gated · Scope: pirate.htb + 10.129.47.117
              </div>
            </div>
          </div>
        )}
